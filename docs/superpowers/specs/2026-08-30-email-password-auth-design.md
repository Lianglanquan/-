...
- 注册不发送邮箱验证码；Resend 只承担密码重置和旧账号兼容所需的邮件发送，API key 只存在后端运行环境。

## 身份与数据模型

`users` 保存账号身份：随机 `id`、规范化邮箱、邮箱查找哈希、密码哈希、角色、账号确认时间、启用状态和时间戳。注册时提交的确认密码只用于一致性校验，不写入数据库。评估数据不直接以邮箱作为外键，而是通过 `assessment_sessions.user_id` 归属到内部用户 ID。

`auth_challenges` 保存密码重置以及旧账号兼容流程的一次性验证码哈希、用途、过期时间、尝试次数和消费时间。验证码不会写入日志或响应体。
...
1. 注册：提交邮箱、密码和确认密码；后端再次校验两次密码一致。邮箱命中管理员白名单时创建为 `ADMIN`，否则创建为 `PARTICIPANT`。
2. 直接登录：账号写入服务器持久化 SQLite 后立即创建服务端会话并返回 `HttpOnly` Cookie，不发送注册验证码。
...
- 首次访问先显示温暖的邮箱登录/注册页；注册页要求两次密码一致，并在成功后直接进入系统。
...
- 确认密码不一致、密码错误、会话撤销、越权读取和安全锁均有测试；密码重置验证码仍覆盖过期和重复使用场景。
