...
export default function AuthFlow({ onAuthenticated }: AuthFlowProps) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        const payload = await callAuth('/api/auth/register', { email, password, password_confirmation: passwordConfirmation })
        if (payload.user) onAuthenticated(payload.user)
      } else if (mode === 'forgot') {
  ...
  const title = mode === 'login' ? '先认出你，再慢慢开始' : mode === 'register' ? '给这段路留一个名字' : mode === 'forgot' ? '密码忘了也没关系' : '换一把新的钥匙'
  const subtitle = mode === 'login'
    ? '每一次回答都会归属于你的私密会话。邮箱只是为了把这段记录好好交还给你。'
    : mode === 'register'
      ? '邮箱只用于找回你的私密会话。设置两次一致的密码，就可以直接开始。'
      : '我们不会透露一个邮箱是否已经注册。'
  ...
  return <main className="auth-scene">
    <section className="auth-card" aria-label="邮箱登录">
      ...
      <form onSubmit={submit} className="auth-form">
        ...
        {mode === 'register' && <label>确认密码<input type="password" value={passwordConfirmation} onChange={(event) => setPasswordConfirmation(event.target.value)} autoComplete="new-password" minLength={8} required placeholder="再输入一次密码" /></label>}
        {mode === 'reset' && <label>邮箱验证码<input inputMode="numeric" pattern="[0-9]{6}" value={code} onChange={(event) => setCode(event.target.value)} required placeholder="6 位数字" /></label>}
        ...
        <button className="auth-submit" type="submit" disabled={loading}>{loading ? '请稍等…' : mode === 'login' ? '进入私密会话 →' : mode === 'register' ? '创建账户并进入 →' : mode === 'forgot' ? '发送重置验证码 →' : '保存新密码 →'}</button>
      </form>
