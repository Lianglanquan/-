      ...
      <div className="sessions-list" aria-label="评估会话列表"><div className="sessions-list-heading"><strong>测试者</strong><span>{sessions.length} 场评估</span></div>
        {sessions.map((session) => {
          ...
          return <button className={`session-row ${session.id === selectedId ? 'selected' : ''}`} type="button" key={session.id} onClick={() => void onSelect(session.id)}>
            ...
            <div className="session-row-meta"><span>{formatDate(session.updated_at)}</span><span>{Number(evidence?.seed_answered ?? 0)} / {Number(evidence?.seed_total ?? 19)} 题</span><span>总分 {report?.total_score == null ? '—' : `${String(report.total_score)}/${String(report.max_score ?? 38)}`}</span><span>{riskLabel(String(report?.risk_level ?? 'INCOMPLETE'))}</span></div>
          </button>
        })}
