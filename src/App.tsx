...
const fallbackQuestions: Question[] = [
  ...
  ['Q16', '如果让我描述一下五年后的今天自己正在做什么，我心里下意识想到的是______。', '自我认知与绝望感'],
  ['Q17', '假如遇到地震或火灾这样的突发危险，我做出的反应是______。', '自我认知与绝望感'],
  ['Q18', '如果我在过马路时，一辆失控的车冲过来，我确定躲不开了，在那一刻我心里涌上来的感觉是______。', '自我认知与绝望感'],
  ['Q19', '如果0分代表“完全不受影响”，10分代表“已经被它渗透到方方面面”，那么近两周那些难受的事情对我的整体生活影响程度，我打______分。', '自我认知与绝望感'],
...
function localScore(questionId: string, response: string): ScoreResult {
  ...
  const score = questionId === 'Q19' && /^\s*(10|[0-9])/.test(response) ? (Number(response.match(/^\s*(10|[0-9])/)?.[1]) > 8 ? 2 : Number(response.match(/^\s*(10|[0-9])/)?.[1]) > 5 ? 1 : 0) : high.some((x) => response.includes(x)) ? 2 : mid.some((x) => response.includes(x)) ? 1 : 0
  const normalized = response.trim()
  const provisional = questionId === 'Q19'
...
function App() {
  ...
  return (
    <main className={`app-shell ${screen === 'welcome' ? 'landing-active' : ''} ${screen === 'assessment' && participantView ? 'participant-active' : ''}`}>
      ...
      {screen === 'welcome' && <WelcomeView totalQuestions={questions.length} onStart={startAssessment} />}
...
function WelcomeView({ totalQuestions, onStart }: { totalQuestions: number; onStart: () => void }) {
  const videoRef = useRef<HTMLVideoElement | null>(null)
  ...
  return <section className={`landing-scene ${leaving ? 'landing-leaving' : ''}`}>
    ...
    <div className="landing-content">
      <div className="landing-kicker"><span className="landing-kicker-dot" /> A QUIET PLACE TO BEGIN <span className="landing-kicker-line" /> <span>01 / {totalQuestions}</span></div>
...
function CompletionView({ answered, submitted, globalEvidence, participantHandoff, nextAction, onContinue, onEvidence, onReplay }: { answered: number; submitted: Record<string, ScoreResult>; globalEvidence: GlobalEvidenceState | null; participantHandoff: ParticipantHandoff | null; nextAction: NextAction; onContinue: () => void; onEvidence: () => void; onReplay: () => void }) {
  ...
  const headline = safety
    ? '先停在这里，剩下的交给专业人员。'
    : unresolved.length > 0
      ? `${globalEvidence?.seed_total ?? answered}句话已经走完，还有几处值得再听听。`
      : humanReview > 0
        ? `${globalEvidence?.seed_total ?? answered}句话已经走完，有些地方先留给专业人员复核。`
        : `${globalEvidence?.seed_total ?? answered}句话已经汇成一张证据地图。`
  const detail = safety
